import { renderer } from "./renderer.js"

const debug = 0

export const animations = {
    activeAnimations: [],
    amount: 0,
    /**
     * @param  {object} animation animation that will be looped trough
     * @param  {} x x coordinate
     * @param  {} y y coordinate
     * @param  {} xVel=0 x velocity
     * @param  {} yVel=0 y velocity
     * @param  {} params={} loop: animation will not disappear after end
     */
    animate: function (animation, x, y, xVel = 0, yVel = 0, params = {}) {
        if (debug)
            console.log(`added animation at ${x} ${y}`)

        this.activeAnimations.push({
            sprites: animation.sprites,
            stage: 0,
            x: x,
            y: y,
            xVel: xVel,
            yVel: yVel,
            params: params
        })
    },
    /**
     * this function will increase all animations stage by 1 
     * and removes animations that are far from screen
     */
    tick: function () {
        this.activeAnimations.map((animation, id) => {
            animation.stage += 1

            const { loop } = animation.params

            if (loop && animation.stage >= animation.sprites.length) {
                animation.stage = 0
            }

            if (animation.x > window.w || animation.x < 0 || animation.y > window.h || animation.y < 0) {
                this.activeAnimations.splice(id, 1)
            }

            if (animation.stage >= animation.sprites.length) {

                if (debug)
                    console.log(`removed animation ${id}`)

                this.activeAnimations.splice(id, 1)
            }

            return animation
        })
    },
    /**
     * this will change the coordinate of animation
     */
    move: function () {
        this.activeAnimations.map((animation, id) => {
            animation.x += animation.xVel / 3
            animation.y += animation.yVel / 3

            return animation
        })
    },
    render: function () {
        this.amount = this.activeAnimations.length
        this.activeAnimations.forEach(animation => {
            renderer.drawObject(animation.sprites[animation.stage], animation.x, animation.y)
        })
    }
}